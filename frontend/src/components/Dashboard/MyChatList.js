        ...
        <Box h={"100%"} px={2} flex={1} overflowY={"auto"} sx={scrollbarconfig}>
          {chatlist.map((chat) => {
            const otherMember = chat.isGroup 
              ? { name: chat.name, profilePic: chat.groupPic } 
            ...
            return (
              ...
              >
                <Button
                  ...
                  w={"100%"}
                  justifyContent={"space-between"}
                  onClick={() => handleChatOpen(chat._id, otherMember)}
                  colorScheme={chat._id === activeChatId ? "purple" : "gray"}
                >
                  <Flex>
                    <Box>
                      <img
                        src={otherMember?.profilePic || "https://via.placeholder.com/150"}
                        alt="profile"
                        style={{
                          width: "40px",
                          height: "40px",
                    ...
                    <Box ml={3} w={"50%"} textAlign={"left"}>
                      <Text
                        ...
                        fontWeight={"bold"}
                      >
                        {otherMember?.name?.length > 11
                          ? otherMember.name?.substring(0, 13) + "..."
